...
export const BookingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateStatus = (id: string, status: 'CONFIRMED' | 'CANCELLED') => {
    const newApps = appointments.map(a => a.id === id ? { ...a, status } : a);
    save(newApps);
  };
  ...
  return (
    <BookingContext.Provider value={{ appointments, addAppointment, updateStatus, getBusinessAppointments, getClientAppointments }}>
      {children}
